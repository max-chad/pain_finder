...
def verify_evidence_spans(quotes: list[str], sources: list[EvidenceSource]) -> list[VerifiedEvidence]:
    return [_verify_quote(quote, sources) for quote in quotes]
...
def _verify_quote(quote: str, sources: list[EvidenceSource]) -> VerifiedEvidence:
    ...
    for source in sources:
        normalized_source = _normalize_for_match(source.text)
...
__all__ = ["EvidenceSource", "VerifiedEvidence", "verify_evidence_spans"]
