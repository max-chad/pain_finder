...
def test_score_prediction_uses_weights_and_penalties():
    ...
    clean = _prediction("clean", useful_factor=1.0, monetizable_factor=1.0, evidence_count=1, penalty=0.0)
...
def test_calibrate_score_writes_deterministic_non_auto_apply_artifact(tmp_path):
    ...
    labels = [
        ...
        {"post_id": "p2", "is_monetizable": True, "feedback_useful": True, "evidence_quality": "multi_quote"},
        {"post_id": "p3", "is_monetizable": False, "feedback_useful": False, "evidence_quality": "no_quote"},
    ]
    predictions = [
        _prediction("p1", useful_factor=0.95, monetizable_factor=0.90, evidence_count=1),
        _prediction("p2", useful_factor=0.82, monetizable_factor=0.75, evidence_count=2),
        _prediction("p3", useful_factor=0.70, monetizable_factor=0.10, evidence_count=0, penalty=0.35),
    ]
    write_jsonl(labels_path, labels)
    write_jsonl(predictions_path, predictions)

    artifact = calibrate_from_files(
    ...
    assert "do not auto-apply" in written["note"].lower()
    assert set(written["recommended_weights"]) == set(CALIBRATION_WEIGHT_KEYS)
    assert written["metric_summary"]["top_n_useful_rate"] == 1.0
    assert written["metric_summary"]["monetizable_precision"] == 1.0
