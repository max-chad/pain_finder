...
def score_prediction(prediction: dict[str, Any], weights: dict[str, float] | None = None) -> float:
    """Score one prediction from visible Wave 5 score components.

    The calibration tool is deliberately read-only: it can recommend weights,
    but it never rewrites runtime configuration or production thresholds.
    """
...
def _label_index(labels: list[dict[str, Any]]) -> dict[str, dict[str, Any]]:
    return {str(row.get("post_id") or ""): row for row in labels if row.get("post_id") is not None}


def _top_joined_rows(
    ...
    *,
    ...
    joined: list[tuple[float, dict[str, Any], dict[str, Any]]] = []
    for prediction in predictions:
        post_id = str(prediction.get("post_id") or "")
        label = labels_by_id.get(post_id)
        if label is None:
            continue
        joined.append((score_prediction(prediction, weights), prediction, label))
    joined.sort(key=lambda item: (-item[0], str(item[1].get("post_id") or "")))
    return joined[: max(0, int(top_n))]
...
def calibrate_from_files(
    *,
    ...
    labels_by_id = _label_index(labels)
    top_rows = _top_joined_rows(labels_by_id, predictions, weights=recommended_weights, top_n=top_n)
    ...
    for _, prediction, label in top_rows:
        components = _score_components(prediction)
        prediction_has_exact_evidence = int(components.get("exact_evidence_count") or 0) > 0
        label_quality = str(label.get("evidence_quality") or "").strip().lower()
        if prediction_has_exact_evidence and label_quality in {"exact_quote", "multi_quote", "linked_multi_source"}:
    ...
    artifact = {
        ...
        "metric_summary": {
            ...
            "evidence_exact_match_rate": _rate(exact_evidence_count, len(top_rows)),
        },
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Write a deterministic, non-auto-apply Wave 5 score calibration artifact.")
